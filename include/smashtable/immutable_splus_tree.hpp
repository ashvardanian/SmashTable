...
class immutable_splus_tree {
  ...
  public:
    ...
    explicit immutable_splus_tree(allocator_t allocator) noexcept : words_(std::move(allocator)) {}
    ...
    /** The bytes a tree over @p keys_count keys stores: whole nodes on every level, each one medium unit. */
    [[nodiscard]] static constexpr std::size_t size_bytes(std::size_t keys_count) noexcept {
        ...
        return nodes_count * format_t::bytes_per_row_k;
    }
