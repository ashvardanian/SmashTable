...
using hashed_core_t = basic_hash_table<value_type_, key_variant_hash_t, key_variant_equal_t, std::allocator<std::byte>>;

template <typename value_type_>
using hashed_monotonic_t = monotonic_store<hashed_core_t<value_type_>>;
template <typename value_type_>
using hashed_snapshot_t = snapshot_store<hashed_core_t<value_type_>>;
...
constexpr store_ops_t hash_map_monotonic_locked = store_bridge<shared_by_lock_t<hashed_monotonic_t<entry_t>>>::table();
constexpr store_ops_t hash_map_monotonic_partitioned =
    store_bridge<shared_by_partition_t<hashed_monotonic_t<entry_t>>>::table();
constexpr store_ops_t hash_map_snapshot_locked = store_bridge<shared_by_lock_t<hashed_snapshot_t<entry_t>>>::table();
constexpr store_ops_t hash_map_snapshot_partitioned =
    store_bridge<shared_by_partition_t<hashed_snapshot_t<entry_t>>>::table();

constexpr store_ops_t hash_set_monotonic_locked =
    store_bridge<shared_by_lock_t<hashed_monotonic_t<key_variant_t>>>::table();
constexpr store_ops_t hash_set_monotonic_partitioned =
    store_bridge<shared_by_partition_t<hashed_monotonic_t<key_variant_t>>>::table();
constexpr store_ops_t hash_set_snapshot_locked =
    store_bridge<shared_by_lock_t<hashed_snapshot_t<key_variant_t>>>::table();
constexpr store_ops_t hash_set_snapshot_partitioned =
    store_bridge<shared_by_partition_t<hashed_snapshot_t<key_variant_t>>>::table();
...
store_ops_t const *hashed_store_ops_for(isolation_choice_t isolation, sharing_choice_t sharing,
    ...
    if (associative) {
        if (snapshot) return partitioned ? &hash_map_snapshot_partitioned : &hash_map_snapshot_locked;
        return partitioned ? &hash_map_monotonic_partitioned : &hash_map_monotonic_locked;
    }
    if (snapshot) return partitioned ? &hash_set_snapshot_partitioned : &hash_set_snapshot_locked;
