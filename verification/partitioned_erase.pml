 ...
 *  @brief A store-level window write spanning every partition of @c partitioned_store, as
 *      @c publish_every_part_ in `include/smashtable/partitioned_store.hpp` makes one.
 *
 ...
 *  against a snapshot needs the stamp alone, so this model defines @c buckets as zero: its claim is
 *  a bucket index nobody keeps and the stamp @c take_snapshot hands back, and @c retire_snapshot
 ...
 *  @c commits 4, @c published_stamp 5, and `landed_at(0..1)` 6 and 7. Left out: @c head, @c members
 *  and @c floors, since the model keeps no census; @c low_water_mark_, since nothing here prunes;
 *  @c generation_, which dates transactions rather than their visibility; and a second committer,
 *  so the ring is never contended here and what @c end_commit does when two commits land at once is
 *  `commit_order.pml`'s.
 */
