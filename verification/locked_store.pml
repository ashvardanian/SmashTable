 ...
 *  @brief Spin model of @c locked_store from `include/smashtable/locked_store.hpp`.
 *
 *  Every call takes the store's shared mutex exactly when it reaches the wrapped store, shared
 *  where it reads and exclusive where it writes, and a transaction's staged state lives
 *  outside the mutex.
 *
