/**
 *  @file verification/commit_order.pml
 *  @author Ash Vardanian
 *  @date September 22, 2026
 *  @brief @c basic_commit_order from `include/smashtable/shared.hpp` under the steps of
 *      `commit_order_steps.pml`.
 *
 ...
 *
 *  `-Dscenario=census`: the buckets, the floors and the mark, against a reader that joins while one
 *  commit lands and a pruner computes, with nothing sequenced between the three.
 *
 *  `-Dscenario=rotation`: the same three roles over two landed stamps, with the reader held back
 *  until the first rotation happened, since a reader racing that first rotation is what @c census
 ...
 *  @c publication. `-Dwithout_done_release` marks the ring relaxed, and the committer that walks
 *  past another's mark carries none of that committer's version to the reader.
 *
 *  Once every commit finished the watermark names every stamp drawn, so a walk that stopped with a
 *  mark behind it is a lost mark, which the auditor catches. `-Dwithout_advance_rmw` re-reads the
 *  watermark with a plain acquire load, and two finishers pass each other's marks under views while
 *  sequential consistency hides it; `-Dwithout_ring_check` lands a stamp @c ring past an
 *  unpublished one, overwriting the older one's mark, which @c land_stamp asserts against as it
 *  stores; that takes `-Dcommitters=3` to reach past a ring of two, and is run under
 ...
 *  watermark and misses a reader that joined in between, which @c census catches;
 *  `-Dwithout_bucket_retag`, which the shared steps also carry, is not claimed by any scenario
 *  here, for the reason `snapshot_reader.pml` gives: a joiner reads its stamp after joining and an
 *  opener stores a floor it read before that, so the release sequence over the watermark puts the
 *  floor at or below the snapshot whichever way the two interleave.
 *
 ...
 *  from was admitted at. `-Dwithout_head_watermark` seals the head against the computed mark, which
 *  equals that bucket's floor by construction wherever that bucket holds the least one, so the head
 *  never moves; `-Dbuckets=1` leaves the head nowhere to rotate to, which is the configuration
 *  `buckets_k >= 2` refuses outright.
 *
 *  The words, and the nine the module allows. @c publication uses up to eight: @c commits 0,
 *  @c published_stamp 1, `landed_at(0..1)` 2 and 3, and `version(0..2)` 4, 5 and 6. The
 *  census scenarios use all nine: @c commits 0, @c published_stamp 1, `landed_at(0..1)` 2 and
 *  3, @c head 4, `members(0..1)` 5 and 6, and `floors(0..1)` 7 and 8. Left out of every
 *  scenario: the @c low_water_mark_ word, since each role acts on the mark it just computed,
 *  which is the freshest such a read can be; and @c generation_, which dates transactions
 *  rather than their visibility.
 */
...
/** How many commits overlap. Two is the least that lets one land before the other, which is what
 *  the ring asks for, and three is what overflows a ring of two; every other scenario watches one
 *  thread publish and needs no overlap at all. */
#ifndef committers
...
/*  The words have to fit the module's @c location_count, which every model here shares. */
#if buckets > 0
 ...
 *  in stamp order and republishes the low-water mark. */
...
/** @c take_snapshot with no census is the watermark read newest; every version that snapshot
 *  covers is written, since its stamp landed before the watermark reached it and the mark that
 *  carried it was released. */
...
/** @c take_snapshot: the bucket joined, then the watermark read newest, and nothing re-read. The
 *  claim is held for the rest of the run, so every mark the committer and the pruner compute beside
 *  it is checked against it; handing a claim back is what @c sharing and @c retention play out. */
active proctype reader() {
    byte joined;
...
/** @c republish_mark_ on a thread of its own, which is the mark a pruning committer computes
 *  without landing a stamp. */
 ...
 *  rotation, and the head it reads may be the one before it, which names that very bucket. Held
 *  back until that first rotation, because a reader racing it is what @c census plays. */
active proctype reader() {
    byte joined;
    int observed, opened, snapshot;
    bool exchanged;
