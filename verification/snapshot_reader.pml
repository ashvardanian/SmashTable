/**
 *  @file verification/snapshot_reader.pml
 ...
 *  that stamp, and gives its claim back after the second read.
 *
 *  Under `-Dscenario=adoption`, a transaction adopts the reader's stamp through @c share_snapshot,
 *  one more member of the bucket the reader is already counted in; the reader gives its own claim
 *  back, and the transaction reads afterwards. Both run on the reader's thread, since
 *  @c transaction_t is constructed from the @c reader_t it adopts, so `commit_order.pml`'s
 *  @c sharing scenario is where a shared member crosses threads.
 *
 ...
 *  @c take_snapshot keeps. `-Dwithout_join_first` reads the watermark first, and a mark computed in
 *  that gap counts nobody while standing above the stamp the reader is handed.
 *
 *  A mark reads the watermark before it scans the buckets, never after. `-Dwithout_watermark_first`
 *  scans first, so a reader joining after the scan is missed by it while the watermark read that
 *  follows has already passed the stamp that reader was handed. A second commit is what raises the
 *  watermark inside that gap, which is what `-Dcommitters=2` supplies.
 *
 *  A stamp adopted from a live claim stays pinned once that claim retires, because the adopting
 *  transaction is counted in the same bucket before the reader leaves it. `-Dwithout_shared_claim`
 *  copies the stamp alone, and the drain that follows lets the mark past it.
 *
 *  A claim retires from a bucket that still counts it, which @c retire_snapshot asserts on the
 *  count its own decrement reads back, so a bucket's live count never borrows from its tag.
 *
 ...
 *  `-Dcommitters=2` supplies under `-Dmemory=sequential` - the shape it turns on is an interleaving
 *  rather than a reordering, so sequential consistency is where it is cheapest.
 *
 ...
 *  @c partition_mutex 0, @c commits 1, @c published_stamp 2, `landed_at(0)` 3, @c head 4,
 ...
 *  is walked. Left out: the @c low_water_mark_ word, as above; @c generation_, which dates
 *  transactions rather than their visibility; and the key's version run, which the partition's lock
 *  orders by itself and which is ghost state rather than a word.
 */
...
/** The reader: takes a snapshot, reads at its stamp and gives its claim back, or hands the stamp
 *  to a transaction. */
...
#endif
#else
    // `share_snapshot`: the transaction counted in the reader's bucket
    // while the reader is still in it
